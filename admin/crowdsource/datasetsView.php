...
<div class="container">
  <div class="col-xs-12 ">
        ...
        <h5>You can update,download or achive the dataset.</h5>
        <div class="heading">
    ...
    <table class="table table-striped" id="currentDatasets">
      ...
      <?php 
          ...
          <tr >
            ...
            <?php if($result[$i]['PUBLISH']==1){ ?> <td><button type="button" class="unpublish btn btn-outline-warning" id="<?php echo $result[$i]['DATASET_ID'];?>"> Unpublish</button></td><?php }else{?>
             <td><button type="button" class="publish btn btn-outline-success" id="<?php echo $result[$i]['DATASET_ID'];?>"> Publish </button></td>
           <?php }?>
            <td><a class="btn btn-outline-info" href="<?php echo "/ubspectrum/admin/crowdsource/progressPageView.php?datasetid=".$result[$i]['DATASET_ID'] ?>"
              class=""> Progress</button></td>
                <td> <a class="btn btn-outline-primary" href="<?php echo "editPageView.php?datasetid=".$result[$i]['DATASET_ID'] ?>"
            class=""> Edit</button></td>
            <td><a class="btn btn-outline-secondary" href="<?php echo '/ubspectrum/admin/crowdsource/file.php?dataset_id='.$result[$i]['DATASET_ID'].'&selectCurrent=true'?>" target="blank" > Download   </button></td>
           <td> <button type="button" class="archive btn btn-outline-danger" id="<?php echo $result[$i]['DATASET_ID'];?>"> Archive</button></td>
        ...
        <h5>You can download or delete the dataset.</h5>
    ...
    <tr>
      ...
      <td><?php echo $list[$i]['DATASET_NAME']?></td>
       <td><?php echo $list[$i]['DATASET_DESCRIPTION']?></td>
        ...
        <td> <button type="button" class="unarchive btn btn-primary" id="<?php echo $list[$i]['DATASET_ID']?>"> Unarchive</button></td>
      <!--   <td><a  class="btn btn-primary" href="<?php echo '/ubspectrum/admin/crowdsource/file.php?dataset_id='.$list[$i]['DATASET_ID']?>" target="blank">Download </a></td> -->
       <!--  <td><a  class="btn btn-primary" href="<?php echo '/ubspectrum/admin/crowdsource/file.php?dataset_id='.$list[$i]['DATASET_ID'].'&delete=true'?>" target="blank">Delete </a></td> -->
